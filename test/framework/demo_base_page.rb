...
class DemoBasePage
  ...
  # visits the page
  def visit(validate_success = true)
    DemoLogger.log.info("Navigating to page #{self.class.name}")
    DemoBaseTest.driver.get(@base_url + @url)
    self.page_loaded? if validate_success
  end
  ...
  def page_loaded?(throw_on_fail = true)
    DemoLogger.log.info("Ensuring we are on the page #{self.class.name}")
